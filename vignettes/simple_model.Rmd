...
**Evaluate the likelihood** We prepare the simulated behavioral data,
compile a model context, and evaluate the log-likelihood under the true
parameter values.
...
**Estimate parameters with `optim()`** We estimate five parameters:
`R1_A.v`, `R1_B.v`, a shared threshold `B` used for both `R1_A.B` and
`R1_B.B`, plus `R2.m` and `R2.s`. We estimate `B` and `s` on the log
scale, to ensure support on the real line.
