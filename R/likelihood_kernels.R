...
.expr_fastpath_supported <- function(expr) {
  ...
  if (identical(kind, "guard")) {
    ref_ok <- .expr_fastpath_supported(expr[["reference"]])
    blocker <- expr[["blocker"]]
    blocker_ok <- is.null(blocker) || .expr_fastpath_supported(blocker)
    unless_list <- expr[["unless"]] %||% list()
    unless_ok <- all(vapply(unless_list, .expr_fastpath_supported, logical(1)))
    return(ref_ok && blocker_ok && unless_ok)
  }
  FALSE
}
...
.expr_cdf_fast <- function(expr, t, prep, component) {
  ...
  if (identical(kind, "guard")) {
    compiled <- .expr_lookup_compiled(expr, prep)
    if (!is.null(compiled) && is.function(compiled$cdf_fn)) {
      return(compiled$cdf_fn(t, component, integer(0), integer(0)))
    }
    return(.eval_expr_cdf(expr, t, prep, component))
  }
  args <- expr[["args"]] %||% list()
...
.expr_survival_fast <- function(expr, t, prep, component) {
  ...
  if (identical(kind, "guard")) {
    compiled <- .expr_lookup_compiled(expr, prep)
    if (!is.null(compiled) && is.function(compiled$surv_fn)) {
      return(compiled$surv_fn(t, component, integer(0), integer(0)))
    }
    return(.eval_expr_survival(expr, t, prep, component))
  }
  args <- expr[["args"]] %||% list()
