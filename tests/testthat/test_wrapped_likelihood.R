testthat::test_that("mapped-to-NA outcome contributes to nonresponse likelihood", {
  spec <- race_spec() |>
  ...
  params <- c(
    go_left.m = log(0.30), go_left.s = 0.20, go_left.q = 0.00, go_left.t0 = 0.00,
    go_right.m = log(0.32), go_right.s = 0.20, go_right.q = 0.00, go_right.t0 = 0.00,
    stop.m = log(0.15), stop.s = 0.18, stop.q = 0.00, stop.t0 = 0.00
  )
  data_df <- data.frame(
    trial = 1L,
    R = NA_character_,
  ...
  expected <- stats::integrate(
    function(t) {
      stats::dlnorm(t - 0.15, params[["stop.m"]], params[["stop.s"]]) *
        (1 - stats::plnorm(t, params[["go_left.m"]], params[["go_left.s"]])) *
        (1 - stats::plnorm(t, params[["go_right.m"]], params[["go_right.s"]]))
    },
    lower = 0.15,
    upper = Inf,
    rel.tol = 1e-6,
