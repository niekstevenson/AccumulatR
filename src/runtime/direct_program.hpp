...
struct DirectProgram {
  ...
  std::vector<semantic::Index> outcome_source_index;
  std::vector<std::uint8_t> outcome_source_kind;
  std::vector<semantic::Index> observed_label_index;

  ParameterLayout parameter_layout{};
};

struct LoweredDirectVariant {
...
struct LoweredDirectModel {
  std::string component_mode{"fixed"};
  std::string component_reference;
  std::vector<LoweredDirectVariant> variants;
};
...
inline void validate_direct_variant(const compile::CompiledVariant &variant) {
  require_direct_variant(variant);

  if (variant.model.outcomes.size() != variant.direct_outcomes.size()) {
    throw std::runtime_error(
        "direct variant has inconsistent outcome lowering metadata");
  }

  for (const auto &leaf : variant.model.leaves) {
    if (leaf.onset.kind != semantic::OnsetKind::Absolute) {
      throw std::runtime_error(
          "direct variant contains chained onset after backend classification");
    }
    if (leaf.trigger_index != semantic::kInvalidIndex) {
      if (leaf.trigger_index < 0 ||
          leaf.trigger_index >=
...
inline Rcpp::List to_r_list(const DirectProgram &program) {
  return Rcpp::List::create(
      ...
      Rcpp::Named("trigger_kind") = as_integer_vector(program.trigger_kind),
      ...
      Rcpp::Named("outcome_source_index") =
          as_integer_vector(program.outcome_source_index),
      Rcpp::Named("outcome_source_kind") =
...
inline LoweredDirectVariant lower_direct_variant(
    const compile::CompiledVariant &variant) {
  detail::validate_direct_variant(variant);

  LoweredDirectVariant lowered;
  ...
  program.layout.n_outcomes = static_cast<int>(variant.direct_outcomes.size());
  ...
  lowered.outcome_labels.reserve(variant.direct_outcomes.size());
  ...
  for (const auto &leaf : model.leaves) {
    ...
    program.parameter_layout.leaf_param_offsets.push_back(
        static_cast<semantic::Index>(program.parameter_layout.leaf_param_slots.size()));
  ...
  program.outcome_source_index.reserve(variant.direct_outcomes.size());
  program.outcome_source_kind.reserve(variant.direct_outcomes.size());
  program.observed_label_index.reserve(variant.direct_outcomes.size());
  for (std::size_t i = 0; i < variant.direct_outcomes.size(); ++i) {
    const auto &outcome = variant.direct_outcomes[i];
    lowered.outcome_labels.push_back(outcome.label);
    program.outcome_source_index.push_back(outcome.source.index);
    program.outcome_source_kind.push_back(
        static_cast<std::uint8_t>(outcome.source.kind));
...
inline LoweredDirectModel lower_direct_model(
    const compile::CompiledModel &compiled) {
  LoweredDirectModel lowered;
  ...
  for (const auto &variant : compiled.variants) {
    lowered.variants.push_back(lower_direct_variant(variant));
  }
  return lowered;
}

inline Rcpp::List to_r_list(const LoweredDirectModel &lowered) {
