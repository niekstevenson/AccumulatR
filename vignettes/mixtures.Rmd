...
- The model controls how mixture weights are defined. Use
  `set_mixture(mode = "fixed")` when the weights are known in advance, and
  `set_mixture(mode = "sample")` when mixture weights should be estimated.
...
trial, with `p.fast` controlling the proportion of fast trials.
...
only the latent mixture weight `p.fast`, while holding the accumulator timing
parameters fixed.
