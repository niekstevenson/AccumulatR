...
inline ExactStepDistributionView evaluate_exact_step_distribution(
  ...
  const auto &compiled_outcome = plan.compiled_outcomes[target_pos];
  step_workspace.transition_probabilities.clear();

  if (!collect_successors &&
      compiled_outcome.total_probability_root_id != semantic::kInvalidIndex) {
  ...
  step_workspace.transition_probabilities.assign(
      compiled_outcome.transitions.size(), 0.0);
  for (std::size_t transition_idx = 0;
       transition_idx < compiled_outcome.transitions.size();
       ++transition_idx) {
