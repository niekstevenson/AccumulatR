...
\Abstract{
  Evidence-accumulation models explain response choices and response times by
  treating behavior as the result of latent processes that race, combine, or
  inhibit one another. Applied work often requires more than a single named
  model family: researchers may want pooled channels, staged onsets, shared
  absence processes, latent mixtures, or observed responses defined by logical
  rules. \pkgname{} is an \proglang{R}/\proglang{C++} package for specifying,
  simulating, and evaluating likelihoods for such structured
  evidence-accumulation models. Model structure is declared in \proglang{R}
  using accumulators, pools, outcome rules, components, triggers, and mixture
  settings. The same finalized model object is then used for simulation, data
  preparation, response-probability calculation, and likelihood evaluation.
  Repeated numerical work is delegated to compiled code via \pkg{Rcpp}. This
  paper describes the software design, the mathematical representation used by
  the likelihood evaluator, and a set of reproducible examples covering basic
  races, pooled outcomes, guarded rules, mixtures, and ranked responses.
}

\Keywords{evidence accumulation, response times, race models, mathematical psychology, \proglang{R}}
\Plainkeywords{evidence accumulation, response times, race models, mathematical psychology, R}
...
Evidence-accumulation models are a central tool for the analysis of choices and
response times in cognitive psychology, neuroscience, and related fields. They
replace a purely descriptive account of accuracy and latency with a process
account: latent evidence streams evolve over time, an observed response is
recorded when one or more streams satisfy a decision rule, and the response time
is the finishing time of the relevant process. Diffusion models, linear
ballistic accumulator models, lognormal race models, and related dynamic choice
models all instantiate this general idea \citep{ratcliffDiffusionDecision2016,
brownSimplestComplete2008, heathcoteWinnerTakes2022, rouderLognormalRace2015}.

The software problem is not exhausted by fitting one standard model family.
Many substantive hypotheses are architectural. A task may require multiple
routes to a response, a response that depends on a pool of channels, a stop or
blocker process, a second stage that can start only after an earlier stage has
finished, a mixture of latent processing modes, or data in which the first two
responses on a trial are both observed. These requirements are common enough
that forcing them into a single winner-takes-all template obscures the model
being tested.

Existing software covers important parts of this landscape. Packages such as
\pkg{RWiener} provide focused implementations for Wiener diffusion models
\citep{wabersichRWienerPackage2014}. Recent \proglang{Stan}-based diffusion
model tools support flexible Bayesian estimation for particular diffusion
parameterizations \citep{henrichSevenparameterDiffusion2024}, while broader
estimation frameworks such as \pkg{lme4} and \pkg{brms} provide mature
interfaces for hierarchical statistical modeling \citep{batesFittingLinear2015,
burknerBrmsPackage2017}. Other work has emphasized hierarchical cognitive
models, approximate likelihoods, and joint neural-behavioral models
\citep{vandekerckhoveHierarchicalDiffusion2011, fenglerLikelihoodApproximation2021,
palestroTutorialJoint2018}. \pkgname{} is complementary to these tools. Its
main contribution is a direct software representation of structured
race-model architectures, coupled to simulation and exact likelihood evaluation.

\pkgname{} follows a compact workflow. The user declares the model structure in
\proglang{R}, finalizes it, expands a named parameter vector into a trial-wise
parameter matrix, and then uses the same finalized model for simulation or
likelihood evaluation. The package is implemented as an \proglang{R} package
\citep{R} with computational kernels in \proglang{C++}, exposed through
\pkg{Rcpp} \citep{Eddelbuettel+Francois:2011}. This paper describes both the
user interface and the model class it implements. Section~\ref{sec:design}
introduces the package objects and current public API. Section~\ref{sec:model}
summarizes the mathematical layer. Section~\ref{sec:workflow} gives the basic
\proglang{R} workflow. Section~\ref{sec:examples} presents structured model
examples. Sections~\ref{sec:likelihood} and~\ref{sec:implementation} describe
likelihood evaluation and implementation details, followed by a comparison to
related software and a discussion.
...
\pkgname{} is not a replacement for specialized diffusion-model packages or
for full estimation systems. Its role is narrower and more structural. A
package such as \pkg{RWiener} provides direct density, distribution, and random
generation functions for the Wiener first-passage-time model
\citep{wabersichRWienerPackage2014}. \proglang{Stan}-based tools provide
Bayesian fitting workflows for particular diffusion parameterizations
\citep{henrichSevenparameterDiffusion2024}. General modeling systems such as
\pkg{lme4} and \pkg{brms} focus on regression and hierarchical estimation
interfaces \citep{batesFittingLinear2015, burknerBrmsPackage2017}. Cognitive
modeling frameworks such as those discussed by
\citet{vandekerckhoveHierarchicalDiffusion2011} address hierarchical
sequential-sampling models from an estimation perspective.

The distinguishing feature of \pkgname{} is that the race architecture itself
is the central software object. The same object is used for simulation,
inspection, response-probability calculation, and likelihood evaluation. This
is useful when the scientific question is whether a response is generated by a
pool, a staged route, a latent mixture, or a guarded process, rather than only
which parameter values best fit a fixed named model.
