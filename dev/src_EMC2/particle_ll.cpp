...
// [[Rcpp::export]]
NumericVector calc_ll_AccR(NumericMatrix p_matrix, DataFrame data, NumericVector constants,
                      List designs, String type, List bounds, List transforms, List pretransforms,
                      CharacterVector p_types, double min_ll, List trend, SEXP context){
  const int n_particles = p_matrix.nrow();
  const int n_trials = data.nrow();
  NumericVector lls(n_particles);
  NumericVector p_vector(p_matrix.ncol());
  ...
  for(int i = 0; i < n_particles; i++){
    ...
    is_ok = c_do_bound(pars, bound_specs);
    lls[i] = cpp_loglik(pars, data, context); // THIS IS WHERE AccumulatR IS CALLED
  }
  return(lls);
}
